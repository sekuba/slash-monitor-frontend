...
export function registerSlashmonServiceWorker(): void {
    ...
    window.addEventListener('load', () => {
        navigator.serviceWorker.register(SERVICE_WORKER_PATH, { scope: SERVICE_WORKER_SCOPE }).catch((error: unknown) => {
            console.error('Unable to register the Slashmon push service worker:', error);
        });
    }, { once: true });
}

export async function getSlashmonServiceWorker(): Promise<ServiceWorkerRegistration> {
    if (!('serviceWorker' in navigator)) {
