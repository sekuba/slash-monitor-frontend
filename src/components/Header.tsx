      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 items-center gap-3 sm:gap-4">
            <div className="shrink-0 bg-vermillion border-5 border-brand-black p-2 sm:p-3 shadow-brutal">
              <svg className="h-8 w-8 text-brand-black stroke-3 sm:h-10 sm:w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="flex flex-wrap items-center gap-3 sm:justify-end">
            {(['monitor', 'pingme'] as const).map((view) => (
              <button
                ...
                className={`brutal-button ${activeView === view ? 'brutal-button--nav-selected' : ''}`}
                aria-current={activeView === view ? 'page' : undefined}
              >
                {view === 'pingme' ? 'PINGME' : view[0].toUpperCase() + view.slice(1)}
              </button>
            ))}
            <a href="https://github.com/sekuba/slashmon" target="_blank" rel="noopener noreferrer" className="brutal-button brutal-button--icon" aria-label="View on GitHub">
