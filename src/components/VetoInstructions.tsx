...
export function VetoInstructions({ payloadAddress }: VetoInstructionsProps) {
    ...
    return (<div className="bg-malachite border-5 border-chartreuse p-6 space-y-4 shadow-brutal-chartreuse">
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          ...
          <p className="text-sm text-whisper-white font-bold mb-4">
            ...
            ({config.slasherAddress ? (<span className="font-mono text-aqua">{config.slasherAddress.slice(0, 6)}...{config.slasherAddress.slice(-4)}</span>) : 'Unknown'})
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={handleCopy} className="px-6 py-3 bg-chartreuse hover:bg-chartreuse/90 border-3 border-brand-black transition-transform hover:-translate-y-0.5 text-sm font-black uppercase shrink-0 shadow-brutal" title="Copy payload address">
                {copied ? (<span className="text-brand-black">COPIED</span>) : (<span className="text-brand-black">COPY</span>)}
              </button>
            </div>
          </div>
        </div>
